...
# → wordpress/dist/giga-pisar-1.0.0.zip (~6 МБ)
```
...
2. Положить архив и ссылку на последнюю версию:
   ...
   cp giga-pisar-1.0.0.zip /var/www/<сайт>/downloads/
   ln -sfn giga-pisar-1.0.0.zip /var/www/<сайт>/downloads/giga-pisar-latest.zip
   sha256sum /var/www/<сайт>/downloads/giga-pisar-1.0.0.zip > /var/www/<сайт>/downloads/giga-pisar-1.0.0.zip.sha256
   ```
...
```bash
wp plugin install /path/giga-pisar-1.0.0.zip --activate --path=/var/www/<wp-сайт>
```
